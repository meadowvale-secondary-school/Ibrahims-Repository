...
let breakfast = mealTime(meal: "Breakfast")

if let meal = breakfast{
    print(meals["Breakfast"]!)
}else{
    print("Meal not recorded")
}

let dinner = mealTime(meal: "Dinner")
...
func checkMealLog() -> [String: Any]?{
    if let myMealLog = UserDefaults.standard.dictionary(forKey: "mealLog"){
        return myMealLog
    }else{
        return nil
    }
...
if let myMealLog = checkMealLog(){
    print(checkMealLog())
}else{
    print("No meal log has been saved.")
}
